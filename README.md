...
индекс брокера = `INDEX − 1`, внешний порт = `1883 + индекс`. Проверочные значения для
конкретного шкафа — `BROKER_ADDRESS` и `BROKER_PORT` в том же файле.
...
docker-compose up -d mosquitto_3                       # пересоздать/дотянуть один сервис
...
девять сразу, а глобальный цикл `down` + `up` уничтожает retained-состояние (см. ниже).
...
Проверено на стенде (изолированный тестовый проект, 2026-09-25):

- перезапуск контейнера (`docker restart`) и `docker-compose up -d --force-recreate`
  состояние сохраняют;
- цикл `docker-compose down` + `docker-compose up -d` — а это ровно то, что делают
  `ExecStop`/`ExecStart` юнита `mosquitto.service` — оставляет прежний анонимный том
  осиротевшим и создаёт новый пустой: `/mosquitto/data` оказывается пустым, retained-сообщения
  теряются.

Практические следствия:

1. Перед `sudo systemctl restart mosquitto` (и перед любым `docker-compose down` в этом
   каталоге) считайте, что retained-состояние всех девяти брокеров будет потеряно.
2. Надёжное решение — объявить в compose именованные тома на `/mosquitto/data` (например,
