...
2. **Что именно теряется.** Единственная публикация с `retain: true` в бэкенде — команда
   `commands/update/deep_sleep` (`smartWard/controllers/mqttController.js`). После пересоздания
   брокеров устройство не получит последнюю команду deep sleep и останется на своём состоянии из
...
4. Тома с данными брокеров (`docker volume ls`) не удалять: `docker-compose down -v`
